...
function EditUser({ data }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (handleEmail() && handleMobile() && handleName()) {
      // no need to send config
      try {
        const config = {
          headers: {
            "Content-type": "x-www-form-urlencoded;charset=utf-8;text",
          },
        };
        console.log('log of here');
        ...
        //   datad = JSON.stringify()

        setLoading(true);
        // console.log("wertyuytfd"+ values._id);
        // const { datas } = await axios.post(
        //   "http://localhost:5002/admin/edituser"+data._id,
        //   {
        //     _id: data._id
            // name,
            // email,
      ...
      await axios.post('http://localhost:5002/admin/edituser',
            {
             _id: data._id,
             name,
             email,
...
console.log("vaaaaaluesss",values._id)

            // console.log(response.data);
        })
        .catch((err)=>{
            console.log(err.message);
            console.log("vaaaaluesss",data._id);
          })
